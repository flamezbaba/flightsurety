...
contract FlightSuretyApp {
    ...
    uint256 constant AIRLINE_FUNDING = 10 ether;
    uint256 constant INSURANCE_PERCENTAGE_MULTIPLIER = 150;

    uint256 constant REGISTER_AIRLINE_REGISTRATION_MAX = 4;
    uint256 constant MULTI_SIG_CONSENSUS_DIVIDER = 2;
    ...
    modifier requireAirlineIsPaid(address airline) {
        require(
            flightSuretyData.isPaidAirline(airline),
            "Only paid airlines are allowed"
        );
        _;
    }
    ...
    modifier requireFee() {
        require(msg.value == AIRLINE_FUNDING, "invalid funding value submitted");
        _;
    }
        ...
        returns (bool success, uint256 votes)
    {
        ...
        if (
            ...
            registeredAirlines.length < REGISTER_AIRLINE_REGISTRATION_MAX
        ) {
            ...
            if (
                ...
                registeredAirlines.length.div(MULTI_SIG_CONSENSUS_DIVIDER)
            ) {
                result = flightSuretyData.registerAirline(name, addr);
    ...
    function refund() external requireIsOperational {
